document.addEventListener('DOMContentLoaded', (event) => {
	const imgSetter = async () => {
		...
		try {
			...
			let commentsBox = (document.querySelector('.comments').innerHTML = '');
		} catch (err) {
			handleError(err);
			loader.innerText = '';
		}
	...
	document.getElementById('upvote').addEventListener('click', (event) => voteFunc('up'));

	document.getElementById('downvote').addEventListener('click', (event) => voteFunc('down'));
